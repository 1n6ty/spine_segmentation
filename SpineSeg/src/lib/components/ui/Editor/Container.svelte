<script lang="ts">
    ...
    async function handleMagicClick() {
        ...
        try {
            const polygons = await runAutofill(sopInstanceUID, arrayBuffer, (status) => {
                magicStatus = status;
            });
            ...
            magicStatus = "done";
            setTimeout(() => { magicStatus = "idle"; }, 800);
        } catch (err) {
            magicStatus = "error";
...
<div class="bg-(--card) text-(--card-foreground) flex flex-col gap-2 rounded-xl border border-(--border) p-4">
    ...
    <div class="flex flex-wrap justify-start gap-x-2 gap-y-2">
        ...
        <Button type="magic" callback={ () => { handleMagicClick(); } } disabled={ magicStatus !== "idle" && magicStatus !== "error" } >{ $t("editor.autofill") }</Button>
            ...
            <div class="absolute inset-0 z-50 flex items-center justify-center bg-(--background)/50 backdrop-blur-sm">
                <div class="bg-(--card) text-(--card-foreground) border border-(--border) shadow-lg rounded-xl p-6 flex flex-col items-center gap-4">
                    {#if magicStatus === "error"}
                        <span class="text-sm font-medium text-(--destructive)">{ magicError ?? $t("editor.loading.error") }</span>
                        <Button type="cancel" callback={() => { magicStatus = "idle"; magicError = null; }}>{ $t('editor.cancel') }</Button>
                    {:else}
