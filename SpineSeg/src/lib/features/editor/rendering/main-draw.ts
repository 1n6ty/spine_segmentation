import { getPolygonCenter } from "$lib/shared/geometry/geometry";
import type { Point, Polygon } from "$lib/shared/geometry/geometry.type";
import { drawCircle } from "./canvas-utils";

function drawBackground(ctx: CanvasRenderingContext2D, image: ImageBitmap, offset: Point, scale: number): void {
...
function drawPolygons(ctx: CanvasRenderingContext2D, polygons: Polygon[], selectedPolygon: Polygon | null, pointsRadius: number, scale: number): void {
...
function drawDraftPoints(ctx: CanvasRenderingContext2D, points: Point[], pointsRadius: number, scale: number): void {
