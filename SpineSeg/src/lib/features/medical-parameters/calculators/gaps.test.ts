...
const S1: Vertebrae = { uuid: "s1", id: "S1", points: [{ x: 70, y: 460 }, { x: 70, y: 400 }, { x: 130, y: 400 }, { x: 130, y: 460 }] };
const L5: Vertebrae = { uuid: "l5", id: "L5", points: [{ x: 70, y: 390 }, { x: 70, y: 330 }, { x: 130, y: 330 }, { x: 130, y: 390 }] };

describe("getGapParams baseline (upright, parallel, no slip)", () => {
    const { params } = getGapParams("side", { top: L5, bottom: S1 }, 1)!;

    it("p1 intervertebral angle is ~0 for two upright, parallel vertebrae", () => expect(params.p1.val).toBeCloseTo(0, 1));
    it("p1 type is angular, per config.ts", () => expect(params.p1.type).toBe("angular"));
    it("p2 anterior disc height is the real ~10px gap, not the combined vertebra heights", () => expect(params.p2.val).toBeCloseTo(10));
    it("p3 posterior disc height is the real ~10px gap", () => expect(params.p3.val).toBeCloseTo(10));
    it("p4 disc wedging angle is ~0 for parallel anterior/posterior borders", () => expect(params.p4.val).toBeCloseTo(0, 1));
    ...
    it("p6 angular displacement is ~90 (border perpendicular to endplate) when there's no slip", () => expect(params.p6.val).toBeCloseTo(90, 1));
});
...
describe("getGapParams p1/p4 sign direction", () => {
    it("p1 is signed (not just magnitude) when the top vertebra's anterior edge tilts relative to the bottom's", () => {
        const L5tilted: Vertebrae = { uuid: "l5c", id: "L5", points: [{ x: 70, y: 390 }, { x: 85, y: 330 }, { x: 145, y: 330 }, { x: 130, y: 390 }] };
...
describe("getGapParams with anterior slip", () => {
    const L5shifted: Vertebrae = { uuid: "l5b", id: "L5", points: [{ x: 75, y: 390 }, { x: 75, y: 330 }, { x: 135, y: 330 }, { x: 135, y: 390 }] };
    const { params } = getGapParams("side", { top: L5shifted, bottom: S1 }, 1)!;

    it("p5 reflects the ~5px shift", () => expect(Math.abs(params.p5.val as number)).toBeCloseTo(5, 1));
});
...
describe("getGapParams L5-S1 special case (p7)", () => {
    ...
    it("is 0 for any non-L5/S1 pair", () => {
        const Th1: Vertebrae = { uuid: "th1", id: "Th1", points: [{ x: 70, y: 200 }, { x: 70, y: 140 }, { x: 130, y: 140 }, { x: 130, y: 200 }] };
        const { params } = getGapParams("side", { top: Th1, bottom: L5 }, 1)!;
        expect(params.p7.val).toBe(0);
    });
});
