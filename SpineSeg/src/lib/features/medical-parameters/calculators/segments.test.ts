...
function vertebraOnCircle(id: string, cx: number, cy: number, R: number, thetaDeg: number): Vertebrae {
    const theta = (thetaDeg * Math.PI) / 180;
    const x = cx + R * Math.sin(theta);
    const y = cy - R * Math.cos(theta);
    const h = 0.5;
    return { uuid: id, id, points: [{ x: x - h, y: y + h }, { x: x - h, y: y - h }, { x: x + h, y: y - h }, { x: x + h, y: y + h }] };
}
...
describe("getSegmentParams on a known circular arc", () => {
    ...
    it("p1 recovers the known radius", () => expect(Math.abs((params.p1.val as number) - R)).toBeLessThan(2));
...
function tinySquare(id: string, cx: number, cy: number): Vertebrae {
    const h = 5;
    return { uuid: id, id, points: [{ x: cx - h, y: cy + h }, { x: cx - h, y: cy - h }, { x: cx + h, y: cy - h }, { x: cx + h, y: cy + h }] };
}
