...
import type { Projection } from "../dicom/types";
import { getGapSagittalParams, getGapFrontalParams } from "./gaps";
import { getSegmentSagittalParams, getSegmentFrontalParams } from "./segments";
import { getSpineSagittalParams, getSpineFrontalParams } from "./spine";
import { getVertebraFrontalParams, getVertebraSagittalParams } from "./vertebraes";

export const getParameters = (projection: Projection, polygons: Polygon[], mmPerPixel: number) => {
    return {
        side: {
            vertebra: {
                pl: polygons.map(v => getVertebraSagittalParams(v, mmPerPixel)),
                en: {
            ...
            disk: {
                pl: polygons.slice(0, -1).map((v, i) => getGapSagittalParams(polygons[i + 1], v, mmPerPixel)),
                en: {
