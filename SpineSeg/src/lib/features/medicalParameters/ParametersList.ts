import { getGapSagittalParams, getGapFrontalParams } from "$lib/features/medicalParameters/gaps";
import { getSegmentSagittalParams, getSegmentFrontalParams } from "$lib/features/medicalParameters/segments";
import { getSpineSagittalParams, getSpineFrontalParams } from "$lib/features/medicalParameters/spine";
import { getVertebraFrontalParams, getVertebraSagittalParams } from "$lib/features/medicalParameters/vertebraes";

import type { StructuralElements } from "$lib/stores/study/study.store";
import type { ParametersListType } from "./ParametersList.type";

type GetParamsType = (side: StructuralElements, frontal: StructuralElements) => ParametersListType;

export const getParameters: GetParamsType = (side, frontal) => {
    return {
        side: {
            vertebra: {
                pl: side.vertebrae.map(v => getVertebraSagittalParams(v)),
                en: {
            ...
            disk: {
                pl: side.gaps.map(g => getGapSagittalParams(g)),
                en: {
            ...
            part: {
                pl: side.segments.map(s => getSegmentSagittalParams(s)),
                en: {
            ...
            overall: {
                pl: [getSpineSagittalParams(side.spine)],
                en: {
        ...
        frontal: {
            vertebra: {
                pl: frontal.vertebrae.map(v => getVertebraFrontalParams(v)),
                en: {
            ...
            disk: {
                pl: frontal.gaps.map(g => getGapFrontalParams(g)),
                en: {
                    p1: { name: "Intervertebral angle", type: "angular" },
            ...
            part: {
                pl: frontal.segments.map(s => getSegmentFrontalParams(s)),
                en: {
                    p1: { name: "Arc radius", type: "linear" },
                    p2: { name: "Arc chord length", type: "linear" },
                    p3: { name: "Arc central angle", type: "angular" },
            ...
            overall: {
                pl: [getSpineFrontalParams(frontal.spine)],
                en: {
                    p1: { name: "Th1-L5 trunk axis inclination angle", type: "angular" },
                    p2: { name: "Th1-L5 trunk axis length", type: "linear" },
                    p3: { name: "GCoM projection (General Center of Mass)", type: "linear" }
