...
export class SessionService {
	...
	async uploadFile(file: File, projection: Projection) {
		...
		this.projections[projection].patient = new PatientService(
			this,
			dicom_parser_lib.parseDicom(new Uint8Array(this.projections[projection].arrayBuffer))
		);
	...
	async requestSave() {
		...
		const bitmap =
			this.projections.side.patient?.study.series.sopInstance.bitmap ||
		...
		const patientUID =
			this.projections.side.patient?.patientUID || this.projections.frontal.patient?.patientUID;
		const patientBirthdate =
			this.projections.side.patient?.birthDate || this.projections.frontal.patient?.birthDate;
		const patientName =
			this.projections.side.patient?.name || this.projections.frontal.patient?.name;
		...
		this.saveTimeout = setTimeout(() => {
			...
			registry.upsert({
				...
				projections: {
					side: {
						hash: $state.snapshot(this.projections.side.hash),
						polygons: $state.snapshot(this.projections.side.polygons)
					},
					frontal: {
						hash: $state.snapshot(this.projections.frontal.hash),
						polygons: $state.snapshot(this.projections.frontal.polygons)
					}
				}
			});
		}, 500);
