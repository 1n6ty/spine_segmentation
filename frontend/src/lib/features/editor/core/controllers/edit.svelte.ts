...
export class EditController {
	...
	history: HistoryController | null = null;
	...
	private commit() {
		...
		this.selectedPolygon =
			this.parent.session.projections[projection].polygons.find((p) => p.uuid === newPoly.uuid) ||
			null;
	...
	deleteSelected = () => {
		...
		this.parent.session.projections[projection].polygons = this.parent.session.projections[
			projection
		].polygons.filter((p) => p.uuid !== selectedUuid);
		this.parent.session.projections[projection].polygons = orderAndName(
			this.parent.session.projections[projection].polygons
		);
		this.selectedPolygon = null;
	...
	handlePointerDown(e: PointerEvent) {
		...
		const worldPoint = screen_to_world(this.getEventClientXY(e), nav.view.offset, nav.view.scale);
	...
	handlePointerUp(e: PointerEvent) {
		if (this.mode === 'drag') {
			...
			if (selectedUuid) {
				this.selectedPolygon =
					this.parent.session.projections[projection].polygons.find(
