...
describe('ToolController central-line integration (Mode 1, end-to-end through the controller)', () => {
	...
	it('dragging a plate-midpoint control point translates both its corner points together', () => {
		...
		// c2's top-plate midpoint sits at (100, 90), clear of any corner (nearest corner is
		// (90, 90) or (110, 90), 10px away -- the control point itself is exactly on target so
		// it wins regardless).
		container.tools.handlePointerDown(
			fake_pointer_event({ clientX: 100, clientY: 90, target: target as any })
		);
		expect(container.centralLine.isDragging).toBe(true);

		container.tools.handlePointerMove(fake_pointer_event({ clientX: 150, clientY: 140 }));
		container.tools.handlePointerUp(fake_pointer_event({ target: target as any }));

		expect(container.centralLine.isDragging).toBe(false);
		// Looked up by uuid, not index -- the drag reshapes c2 enough that `orderAndName`'s
		// TSP-by-centroid-Y reorder (on commit) can legitimately swap array order.
		const draggedC2 = session.projections.side.polygons.find((p) => p.uuid === c2.uuid)!;
		expect(draggedC2.points).toContainEqual({ x: 140, y: 140 });
		expect(draggedC2.points).toContainEqual({ x: 160, y: 140 });
		expect(target.releasePointerCapture).toHaveBeenCalled();
	});
	...
	it('a plain vertex click still wins over a nearby control point when the vertex is nearer', () => {
		...
		// c2.points[1] === {x: 90, y: 90}, exactly on target; c2's top-plate control point at
		// (100, 90) is 10px away -- the vertex, being nearer, must win the dispatch.
		container.tools.handlePointerDown(
			fake_pointer_event({ clientX: 90, clientY: 90, target: target as any })
		);
		...
		const draggedC2 = session.projections.side.polygons.find((p) => p.uuid === c2.uuid)!;
		expect(draggedC2.points).toContainEqual({ x: 200, y: 210 });
	});
	...
	it('central-line control points are inert outside the select tool', () => {
		...
		container.tools.handlePointerDown(fake_pointer_event({ clientX: 100, clientY: 90 }));
