<script lang="ts">
	...
	import type { PatientInfoBlock } from '$lib/components/sections/patient-info/PatientInfo.type';

	import InfoBlock from '$lib/components/sections/patient-info/InfoBlock.svelte';
	...
	let info: PatientInfoBlock[] = $derived([
		{
			...
			subBlocks: [
				...
				},
				{
					...
					comment:
						project.session.mergedPatient?.birthDate?.toLocaleDateString(
							$locale || undefined,
							dateTimeOptions
						) ?? $t('not_found')
				},
				{
					...
					comment: `${project.session.mergedPatient?.sex ?? $t('not_found')} • ${get_age(project.session.mergedPatient?.birthDate) ?? $t('not_found')}`
				}
			]
		},
		{
			...
			subBlocks: [
				{
					...
					comment:
						project.session.mergedStudy?.studyDate?.toLocaleDateString(
							$locale || undefined,
							dateTimeOptions
						) ?? $t('not_found')
				},
	...
	{#each info as iblock}
		<div>
			<h3 class="mb-4 flex items-center gap-2 font-semibold">
				<img src={iblock.icon} alt={iblock.alt} class="h-4 w-4" />
