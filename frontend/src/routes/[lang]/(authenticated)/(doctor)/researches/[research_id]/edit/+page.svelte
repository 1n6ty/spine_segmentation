...
{#await project.session.loadingPromise then}
	<div class="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2">
		...
		{/if}
		{#if project.session.projections.frontal.patient}
			<XrayEditorCard projection="frontal" />
		{/if}
	</div>
	<div class="rounded-lg bg-(--muted) p-4">
		<h3 class="mb-2 font-semibold">{$t('editor.instructions.header')}</h3>
		<ul class="space-y-1 text-sm text-(--muted-foreground)">
			{#each instructions[$locale && $locale in instructions ? $locale : 'en'] as inst (inst)}
				<li>{inst}</li>
			{/each}
		</ul>
	</div>
