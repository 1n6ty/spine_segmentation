...
class PVertebrae(Vertebrae):
    ...
    def __init__(self, mask_xy: np.ndarray[np.int32] | None = None, reference_points: np.ndarray[np.int32] | None = None) -> None:
        ...
        if not (mask_xy is None):
            ...
            if reference_points is None:
                self.reference_points: np.ndarray[np.int32] = PVertebrae._compute_reference_points(mask_xy)
    ...
    def _compute_reference_points(mask_xy: np.ndarray[np.int32]) -> np.ndarray[np.int32]:
        ...
        weights: np.ndarray[np.float32] = np.array([PVertebrae._compute_cnt_weight(cnt, i) for i in range(cnt.shape[0])], dtype=np.float32)
        while cnt.shape[0] > 4:
            ...
            weights[min_weight_index % cnt.shape[0]] = PVertebrae._compute_cnt_weight(cnt, min_weight_index % cnt.shape[0])
            weights[min_weight_index - 1] = PVertebrae._compute_cnt_weight(cnt, min_weight_index - 1)
