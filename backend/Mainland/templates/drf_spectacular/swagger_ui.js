...
const isWsOperationPath = (path) => typeof path === "string" && path.includes("::");

const isWsOperationProps = (props) => {
  if (isWsOperationPath(props.path)) {
    return true;
  }
  const specPath = props.specPath;
  if (specPath && typeof specPath.get === "function" && isWsOperationPath(specPath.get(1))) {
    return true;
  }
  return false;
};

const hideExecuteForWsOperations = () => ({
  wrapComponents: {
    Execute: (Original, system) => (props) => {
      try {
        if (isWsOperationProps(props)) {
          return null;
        }
      } catch (e) {
        console.error("hideExecuteForWsOperations (Execute) plugin failed:", e);
      }
    ...
    TryItOutButton: (Original, system) => (props) => {
      try {
        ...
        console.error("hideExecuteForWsOperations (TryItOutButton) plugin failed:", e);
      }
      return system.React.createElement(Original, props);
    },
  },
...
plugins.push(hideExecuteForWsOperations);
...
const requestInterceptor = (request, ...args) => {
  ...
  if (!["GET", undefined].includes(request.method) && request.credentials === "same-origin") {
    request.headers["{{ csrf_header_name }}"] = getCsrfCookie();
  }
  return request;
};
